...
<input name="someUniqueName" value={this.cities.someState} onChange={this.handeInputChange} /> */

//oninput: {span type="search" id="b"}
...
/* function myFunction() {
   ...
   // document.getElementsByClassName("suggestions").innerHTML = x;
   //document.querySelectorAll('.suggestions').innerHTML=x;
...
const cities = [ // here we export the array cities to use it in another file
 ...
 // FORMLARLA CALISIRKEN ID VEYA ATTRIBUTE KULLAN
 // we use name attributes for search forms and input or id ile calisiyor...
 const citiesForm= document.forms.searchForm;//
 const citiesInput= citiesForm.searchedCity;
 const outputList=document.querySelector('.suggestions');
 //highlight yapmak icin function
 function marking(str, regx){
   return str.replace(regx, `<mark>$&</mark>`);//replace method =$& // for highlight

 }

 //function payAttention(num){return num.toString().replace()}

 

 /* citiesForm.addEventListener('', ()=>{}); */


 citiesForm.addEventListener('input', (e)=>{
     const searchValue=e.target.value;
     console.log(searchValue);
     const regex= new RegExp(searchValue, 'ig');
     /* const matches= cities.map(stadt=> {// stadt her objecti temsil eidyor
        const citiesFound=stadt.city.match(regex);
       
        return citiesFound;
     });
    const newMatches=[];
    matches.map(element=> {if(element!==null)
...
const newMatches = cities.filter(element=>element.city.match(regex));
       ...
       const html=newMatches.map(element=>{
         return `<li>${marking(element.city, regex)}, ${element.state} ${element.population}</li>`;
       }); // marking highlight yapiyr 
    console.log(html);
    outputList.innerHTML=html.join('');
    });